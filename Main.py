...
# ================= BOT TOKENI VA BOT EGASI (OWNER) ID =================
TOKEN = os.environ.get('BOT_TOKEN', '8816866283:AAGJK1TXHj1b7LZQYQOG7e5w18fOfUH51PM')
bot = TeleBot(TOKEN)

# 👑 SIZNING TELEGRAM ID'INGIZ OʻRNATILDI:
OWNER_ID = 6090422473 

# MEDIA FILE_ID'LAR (8 TA ANIMATSIYA/VIDEO)
...
def start_cmd(message):
    bot.reply_to(message, "👋 **Death Note Botiga xush kelibsiz!**\n\n🎮 `/create` — Guruhda o'yin yaratish\n🛒 `/shop` — Buyumlar do'koni\n🏆 `/top` — Eng kuchli o'yinchilar reytingi\n🎯 `/achievements` — Maxsus topshiriqlar va yutuqlar")

# 👑 FAQAT SIZ (OWNER) ISHLATA OLADIGAN COIN QO'SHISH BUYRUG'I
@bot.message_handler(commands=['addcoins'])
def add_coins_cmd(message):
    if message.from_user.id != OWNER_ID:
        bot.reply_to(message, "⛔ Bu buyruq faqat bot egasi uchun amal qiladi!")
        return
    ...
    try:
        ...
        bot.reply_to(message, f"✅ Foydalanuvchi `{target_id}` hisobiga **{amount} coin** qo'shildi!\nYangi balans: **{profile['coins']} coin**")
    except Exception:
        bot.reply_to(message, "⚠️ Qoida: `/addcoins <user_id> <miqdor>` shaklida yuboring.\nMasalan: `/addcoins 6090422473 500`")

@bot.message_handler(commands=['top'])
def top_cmd(message):
    if not user_data:
        bot.reply_to(message, "🏆 Hali hech kim reytingga kirgani yo'q!")
        return

    sorted_users = sorted(user_data.values(), key=lambda x: x['wins'], reverse=True)[:10]
    text = "🏆 **TOP-10 Eng Kuchli O'yinchilar:**\n\n"
...
def achievements_cmd(message):
    ...
    text = f"🎯 **{profile['name']}ning Yutuqlari:**\n\n"
    text += f"{'✅' if ach['first_win'] else '❌'} **Birinchi G'alaba:** O'yinda 1 marta yutish (+50 coin)\n"
    text += f"{'✅' if ach['serial_killer'] else '❌'} **Master Kira:** O'yinda 3 ta qurbonni yo'q qilish (+100 coin)\n"
    bot.reply_to(message, text)
